...
module test_coin::coin_tests {
    ...
    use std::string::{Self, String};  // 添加 string 模組
    ...
    fun test_mint_and_transfer() {
        ...
        test_scenario::next_tx(&mut scenario, USER1);
        {
            ...
            let coin_num = coin::get_value(&coin);

            // Debug: Print USER1's coin number
            std::debug::print(&coin_num);

            assert!(coin_num == 50, 1);
    ...
    fun test_burn_coin() {
        ...
        test_scenario::next_tx(&mut scenario, ADMIN);
        {
            coin::mint_coin(ADMIN, test_scenario::ctx(&mut scenario));
        };
